...
export default function BaseStatusBoard({ reports, unit }) {
  const bases = useMemo(() => {
    const seen = new Set()
    return reports.filter(r => { if (seen.has(r.base)) return false; seen.add(r.base); return true }).map(r => r.base)
  }, [reports])
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {bases.map(base => {
          const { status, last, daysSince } = getBaseStatus(reports, base)
